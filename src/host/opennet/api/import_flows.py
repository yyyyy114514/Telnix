...
def _opennet_flow_to_flow(flow: dict, session_id: int) -> dict:
    """OpenNet 导出的 flow 字典（可能含 id/session_id 等额外字段）转可插入的 flow。"""
    # 允许缺失字段，用默认值补齐
    return {
        "session_id": session_id,
...
async def import_flows(body: ImportRequest = Body(...)):
    ...
    if fmt == "har":
        ...
        for f in flows_list:
            flows_to_insert.append(_opennet_flow_to_flow(f, session_id))
