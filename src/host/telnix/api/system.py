...
def _read_actual_proxy_enabled() -> bool:
    ...
    非 Windows 平台：无注册表，返回 False（代理监控在非 Windows 上不生效）。
    """
    if sys.platform != "win32":
        # 非 Windows 平台：无注册表可读，直接返回 False
        return False
    try:
        import winreg
...
def _proxy_monitor_loop():
    ...
    非 Windows 平台：直接退出（无注册表可监控，代理状态由用户手动管理）。
    """
    global _proxy_lost
    # 非 Windows 平台：不监控代理状态（无注册表，代理由用户手动配置）
    if sys.platform != "win32":
        return
...
def _do_shell_elevate() -> tuple[bool, str]:
    ...
    非 Windows 平台：UAC 是 Windows 专属机制，直接返回错误。
    macOS/Linux 用户需用 sudo 手动以 root 身份运行。
    """
    if sys.platform != "win32":
        return False, ("restart-as-admin 仅 Windows 支持（UAC 提权），"
                       "macOS/Linux 请用 sudo 手动以 root 身份运行 Telnix")
