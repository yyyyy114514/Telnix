...
const canStart = computed(() => isWindows.value && isAdmin.value)
      ...
      <div class="dns-card" v-loading="loading">
        <div class="dns-card-header">
          <div class="dns-card-title">
            ...
            <div class="title-sub">WinDivert 网络层 · 修改本机 DNS 响应中的 A 记录</div>
          </div>
          <el-switch
            :model-value="running"
            :loading="toggling"
        ...
        <div class="dns-card-body">
          <div v-if="!isWindows" class="dns-hint warn">
            <el-icon><WarningFilled /></el-icon>&nbsp;当前平台不支持（WinDivert 仅 Windows 可用）
          </div>
          <div v-else-if="!isAdmin" class="dns-hint warn">
            <el-icon><WarningFilled /></el-icon>&nbsp;需要管理员权限，请用管理员身份重启 Telnix
          </div>
          <div v-else-if="status?.last_error" class="dns-hint err">
            <el-icon><CircleCloseFilled /></el-icon>&nbsp;{{ status.last_error }}
          </div>
          <div v-else-if="running" class="dns-hint ok">
            <el-icon><CircleCheckFilled /></el-icon>&nbsp;劫持运行中，规则已生效
          </div>
          <div v-else class="dns-hint">
      ...
      <div class="dns-card">
        <div class="dns-card-header">
          <div class="dns-card-title">
            ...
            <div class="title-sub">域名 → IP 映射；支持 <code>*.example.com</code> 通配；留空则跳过</div>
          </div>
          ...
          <div class="rules-table">
            ...
            <div v-for="(r, i) in rules" :key="i" class="rule-row">
              <div class="rule-cell c-domain">
                <el-input v-model="r.domain" size="small" placeholder="example.com 或 *.example.com" @input="onRuleInput" />
              </div>
