...
const knownHosts = computed(() => {
  ...
  store.flows.forEach((f: any) => { if (f.host) set.add(f.host) })
  return [...set].sort()
})

// 已出现的方法列表
const knownMethods = computed(() => {
  const set = new Set<string>()
  store.flows.forEach((f: any) => { if (f.method) set.add((f.method as string).toUpperCase()) })
  return [...set].sort()
})

// 已出现的状态码列表
const knownStatusCodes = computed(() => {
  const set = new Set<string>()
  store.flows.forEach((f: any) => {
    if (f.status_code !== null && f.status_code !== undefined) set.add(String(f.status_code))
  })
  return [...set].sort()
})
