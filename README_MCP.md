...
> 全平台可用。网络层功能各平台使用不同后端，接口一致。

| 工具 | 说明 |
|---|---|
| `proxy_status` | 系统代理状态（全平台，各平台读取实际代理状态） |
| `proxy_on` / `proxy_off` | 系统代理开关（Windows 用 winreg，macOS 用 networksetup，Linux 用 gsettings/kwriteconfig5） |
| `cert_status` | 证书状态 |
| `cert_install` / `cert_remove` | HTTPS 证书管理（mac 用 `security`，Linux 需手动） |
| `focus_status` / `focus_on` / `focus_off` | 专注模式（只抓指定进程/host） |
| `raw_capture_status` / `raw_capture_start` / `raw_capture_stop` / `raw_capture_install` | TCP/UDP 原始抓包（Windows 用 WinDivert，macOS 用 BPF，Linux 用 AF_PACKET；`install` 仅 Windows 需要） |
| `system_restart` / `system_quit` | 系统重启/退出 |
| `system_restart_as_admin` | 管理员提权重启（Windows 用 UAC，macOS 用 osascript，Linux 用 pkexec/sudo） |
...
网络层重定向出站 HTTP(80)/HTTPS(443) 流量到本地代理，应用无需配置代理即可被抓包。全平台可用，需管理员/root 权限。Windows 用 WinDivert NAT，macOS 用 pf rdr，Linux 用 iptables REDIRECT。启动失败时返回 `hint` 提示调用 `system_restart_as_admin`。

| 工具 | 说明 |
|---|---|
| `transparent_proxy_status` | 查看状态（running/redirected_count/nat_table_size/last_error/supported/backend/is_admin） |
| `transparent_proxy_start` | 启动透明代理（需管理员/root，失败给明确错误） |
| `transparent_proxy_stop` | 停止透明代理 |
...
3. transparent_proxy_start → 启动透明代理重定向（Windows: WinDivert / macOS: pf / Linux: iptables）
4. （用户操作目标应用触发请求）
