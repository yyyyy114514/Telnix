...
- `raw status` 返回字段：`is_admin`（是否管理员）、`pydivert_installed`（pydivert 是否已装）、`running`（是否在抓）。
- `raw install`：调后端 `/raw/install-pydivert` 执行 `pip install pydivert`。安装后 `raw status` 的 `pydivert_installed` 变为 true。
- `--pid`：只抓指定 PID 的包（可多次）。
- `--port`：只抓指定端口的包（可多次）。
- `--filter`：WinDivert BPF 过滤表达式（默认 `tcp or udp`）。
...
- `system restart-as-admin`：**以管理员身份重启，走 GUI 用户确认流程**：
  ...
  4. 用户点「否」→ CLI stderr 输出 `{"ok": false, "rejected_by_user": true, ...}` + 退出码 1
